...
def cleanup_old_auth_tokens(auth_token_dir):
    ...
    for file_name in os.listdir(auth_token_dir):
        ...
        if now - tok_time > MAX_AUTH_TOKEN_AGE:
            print('Removing auth token ' + file_name + ' because it is ' +
                  str(now - tok_time - MAX_AUTH_TOKEN_AGE) + ' seconds too old.')
            os.remove(tok_path)
    ...
    for (tok_path, tok_time) in old_toks:
        print('Removing auth token ' + os.path.split(tok_path)
              [1] + ' because there are too many auth tokens.')
        os.remove(tok_path)
...
def check_auth_token(auth_token_dir, auth_token, requester, state):
    ...
    if not is_hex_string(auth_token):
        return False
...
def get_auth_state_identity_server(auth_state_dir, auth_state):
    ...
    if not is_hex_string(auth_state):
        return None
