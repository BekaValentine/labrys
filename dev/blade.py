...
PRIVATE_SIGNING_KEY_FILE = os.path.join(
    IDENTITY_DIR, 'private_signing_key.txt')
...
def identity_authenticate_post():
    ...
    if not ('requester' in request.form and 'state' in request.form and 'return_address' in request.form):
        return redirect('#')
...
def identity_verify():
    ...
    if not (auth_token and requester and state):
        return 'bad request', 400

    auth.delete_auth_token(AUTH_TOKEN_DIR, auth_token)

    if not (auth.check_auth_token(AUTH_TOKEN_DIR, auth_token, requester, state)):
        return 'unauthorized', 401
